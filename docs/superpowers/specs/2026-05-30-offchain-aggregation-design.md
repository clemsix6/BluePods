...
This last point is a prerequisite refactor, not a property the code has today, and it needs two extractions. The light validator types (`ValidatorSet`, `Hash`, `ValidatorInfo`) live in `internal/consensus` alongside the entire DAG, so they move into a small shared package, for example `internal/validators/`. And the pure attestation helpers the daemon needs (rendezvous hashing, BLS aggregation and verification, the object-hash function) currently live in `internal/aggregation`, which imports `internal/state` and `internal/network`; they move into a leaf package, for example `internal/attest/`, that imports only the crypto libraries and `internal/validators`. Only then can the daemon import rendezvous and BLS without dragging in storage, the WASM runtime, or the network layer. When the first external non-monorepo consumer appears, the daemon and that shared package move into their own module, at which point the shared package must leave `internal/` because `internal/` packages are not importable across modules; until then it stays under `internal/`.
...
Going QUIC only is more than removing the client API. One inter-node path runs over HTTP today even though the validators already hold a persistent QUIC connection to each other: object retrieval. When a node needs an object it does not hold, it fetches it from a holder over that holder's HTTP endpoint instead of the existing mesh. This moves onto QUIC as a `GetObject` message reusing the mesh and the QUIC address validators already publish. And the legacy HTTP address is removed from the validator's on-chain identity: `HTTPAddr` disappears from the register-validator transaction arguments, the `Validator` type, the validator set, and the snapshots. That is a consensus-breaking change, on par with removing the aggregator fee field, acceptable before mainnet. Validators publish only a QUIC address.
