...
This is one design of record, implemented as one chantier on the `verifiable-indexing` branch, decomposed into ordered plans. Its one prerequisite (section 2) has already landed on `main`.
...
- The anchor is decided by evidence over one stake set, so the verdicts cannot fork: *support* is a round-N+1 vertex listing a specific vertex of the designated producer among its direct parents. The certified vertex is selected by the citations themselves: the producer's vertex reaching the stake quorum of round N+1 is the anchor, so an equivocating producer cannot make two nodes certify different vertices; a vertex citing a different vertex of the same producer abstains, and the round is **skipped** only when a quorum cites no vertex of the producer at all. Both quorums are over the same round's stake, so they intersect and are mutually exclusive. An anchor with split evidence is decided **indirectly** by the first later decided anchor: committed if it sits in that anchor's causal history, skipped otherwise, so an undecided round can never wedge the pipeline. The relaxed bootstrap regime is bounded deterministically, never by a node's local join timing: relaxed until `strictStartRound`, a latch derived from committed history (the round at which the minimum validator count of committed members hold a committed non-zero self-stake, plus the transition grace and buffer windows; arming on stake, not registration, keeps the strict quorum reachable the moment the regime turns strict), persisted and carried in sync snapshots. During relaxed rounds the candidate is still selected by citations under the existing relaxed certificate, never by a locally-chosen lowest-arrived-hash rule, which is view-dependent and can fork two honest nodes on a delivery gap; at the latch the registered set is frozen from committed registrations as the genesis holder snapshot, so anchor designation and stake quorums never read a live, mutating validator set. Total latency stays in the N+2 envelope the current commit rule already has.

This landed on `main` before the indexing work began: the anchor rule with divergence tests (two nodes fed the same vertices in different orders produce identical committed logs), then a scenario-corpus bug-fix campaign that hardened it (certification-impossibility and long-silence rules resolve undecided runs a dead producer left behind; relaxed resolution waits until the round's evidence is view-independent; deep catch-up gaps close through a vertex-range fetch).
...
**Creation is permissioned.** A KeyRoot parent may be any key. Creating an object under a key is a gift: the creator pays the deposit, the same consent-free attach the transfer operation already allows, and the faucet and one-transaction payments (a split paying the recipient directly) depend on it. An ObjectParent must be under the sender's control: an object whose parent walk terminates at the sender's key, an object created earlier in the same transaction, or an object this transaction references through a domain reference (the existing shared-access exemption). Without the object-parent rule anyone could attach junk inside another owner's provable subtree. Enforcement runs at commit during creation, which every node already executes.
...
- **A transaction is either declared operations or a pod call, never both.** This keeps each transaction's semantics atomic without asking non-holders to observe a pod revert: a reverted pod call cannot strand a half-applied reparent because the two cannot share a transaction. The exclusivity extends to `created_objects_replication`: only a pod-creating transaction has objects to replicate, so an operations transaction declaring replication entries is rejected — at client ingress and again on the commit path through one shared shape gate, so the rule binds byzantine producers and forged submissions, not only polite clients. Without it the deposit those entries price would be debited with no object to lock it on and no path to the pool; as a fail-safe, a deposit withheld from a rejected operations list is pooled, never dropped.
- **The operations list is bounded by the same cap as the reference list.** Each operation is priced, but a bound keeps the SMT work one transaction can impose proportionate to the reference-capped baseline; the same shared gate rejects a longer list at both sites — the bound protects work done before any fee is charged, so an ingress-only bound would not bind. A pod may still set the parent of an object it creates (creation is globally executed); it may never change the parent of an existing object. `applyUpdatedObjects` rejects any updated object whose parent bytes differ from the ATX input object's, a purely local compare.
...
**Validator tree: a flat SMT, frozen per epoch.** Key `BLAKE3(validatorPubkey)`, value `{pubkey, cappedStake, blsKey, status}`, rebuilt at each epoch boundary from the same snapshot that freezes `epochHolders`. During the genesis epoch, which freezes no snapshot, the tree tracks the committed registration set, refrozen at each committed registration or bond, the same committed-only source the genesis holder snapshot uses; a live, view-dependent set would fork the anchored root, and a set frozen once at boot would diverge between a restarted node and its never-restarted twin. The first frozen tree is taken at the first boundary. This is what lets a light client verify a stake quorum without trusting the serving node (section 5).
...
**Where the client's validator set comes from.** The validator tree closes the circularity: epoch N's quorum-attested root commits to epoch N+1's validator set, so a client that trusts one checkpoint can walk forward across epoch boundaries by reading each new set out of the index with a proof, exactly the light-client pattern of other proof-of-stake systems. The handoff at a boundary is explicit: headers whose epoch is N+1 but whose frontier falls within the boundary window are weighed by epoch N's tree (validator churn is capped per epoch, so the sets overlap by construction), and the first epoch-N+1-attested root then proves the new set for all later frontiers. Bootstrap trust is a configured checkpoint obtained out of band; this is standard weak subjectivity, stated here explicitly rather than assumed silently. A light client pins `(epoch, indexRoot, validator set hash)`. The load-bearing component is the validator set hash: it authenticates the committee the client's first quorum is weighed by, and every later committee comes out of a quorum-attested root. The pinned index root serves first contact — when the serving node still holds that root, the committee is authenticated by the root directly; once the chain has moved past it (the ordinary case), the client falls back to the set hash, and proofs are always checked against a fresh attested root, never the stale pin. A joining node pins `(epoch, validator set hash)` only. It cannot recompute a past frontier's index root — its snapshot is pinned at the source's last committed round and carries no versioned history — so pinning one would either refuse every honest join under load or, weakened to "the root appears somewhere in the attested history", be satisfied for free by a source copying the public `GetIndexAnchor` value into its own headers. The validator-set hash is the component a joiner can recompute from the snapshot it was served, and section 9's quorum closes the state leg against it.
...
- **Ingress:** if a received vertex anchors a frontier at or below the receiver's own committed frontier (the common case), the receiver verifies the root against its retained history, a 32-byte compare. A mismatch is quarantined, not dropped: the vertex is stored for causal completeness and served on request, but never relayed and never referenced. Terminal rejection would be a partition lever — a vertex some nodes refuse to store, smuggled into committed causal history through colluding references, stalls those nodes' commit forever, and any node's causal batch must remain completable from its own store. A vertex anchoring a frontier the receiver has not reached yet is accepted without buffering; blocking the door on it would couple vertex acceptance to commit lag and slow the DAG under skew. A vertex anchoring a zero root is tolerated only during the genesis epoch, where no index exists yet; from the first epoch boundary on, an empty anchor is quarantined like a wrong one.
- **Production (the teeth):** a producer never selects as parent a vertex whose anchor it has PROVEN wrong (its own retained history contradicts the claim); anchors it cannot verify yet are referenceable, or ordinary commit skew would silence a producer whose index trails its peers. A wrong-root vertex is therefore never referenced by any honest node able to check it, and can enter committed history only through references from nodes that could not check it or chose not to — in which case the commit stage convicts it everywhere.
...
There is therefore **no index-specific slashing**. A lie is worthless: it is never referenced by a node able to check it, it cannot fool a client (who requires a stake quorum a minority cannot forge), and if it reaches committed history at all it convicts its author with his own signature on every node. The conviction guarantee covers frontiers the network actually reaches: an anchor at a frontier no node ever commits is never re-checked and never convicted — it is also never load-bearing, since bundles and clients only consume roots matched against committed history. The index sits downstream of execution integrity (the open fraud-proof problem already noted in the whitepaper) and neither solves nor worsens it.
...
**Name alphabet.** A name is structurally valid iff it is dot-separated labels, each a non-empty run of lowercase ASCII letters, digits, and hyphens; anything else is rejected at registration. Matching stays byte-exact FCFS: restricting the alphabet removes the confusable-name surface (no case pairs, no lookalike scripts) without introducing a normalization function into consensus — a rejection rule cannot drift, a folding rule can.

**Every leaf write is priced.** Register and renew price their writes through rent; update, transfer, and delete rewrite a leaf without paying rent, so each carries a flat per-operation fee — an unpriced write would be a free SMT rewrite on every node.
...
**Ownership and namespaces.** The leaf's `owner` is the registrant, or whoever a `domain_transfer` handed the name to. Renewal, update, transfer, and deletion require `sender == owner`; during the grace window only the owner may renew. Registering a dotted name requires owning the immediate parent name (`x.y` requires owning `y` — the parent is the SUFFIX), which recursively makes the first-come-first-served namespace rule real for the first time; bare roots are first-come-first-served, and the `system` namespace stays reserved (as built: the bare root `system` is unregistrable, which under the suffix rule blocks every `*.system` name, and names starting with the literal `system.` label are rejected as well — both directions closed). All checks are point lookups in the domain tree at commit, deterministic on every node. The whitepaper's current namespace wording states the opposite (first-label) direction and is rewritten by the documentation batch to match this rule.
...
**Default parameters** (mechanism fixed, numbers calibratable at review): `rental_rate` per epoch quoted relative to the base compute fee; `max_term_epochs`; a grace window of a small number of epochs; `index_entry_fee`; flat per-operation fees for reparent, delete, and the rent-free domain operations (`domain_update`, `domain_transfer`, `domain_delete`) alongside the existing `min_gas`. These are proposed defaults, not load-bearing constants.
...
- `bpctl` accepts a trust checkpoint (persisted alongside the wallet); when present, its index reads go through the verification library and are proved, otherwise they are honest unproven reads of the answering node.
...
Remove: the dead `pods/pod-sdk/src/domain_generated.rs` and its `TrieNode`/`DomainRegistry` types; `pods/pod-sdk/src/vertex_generated.rs`, orphaned the same way (on disk, never declared as a module); the `registered_domains` field of `PodExecuteOutput` and `MaxCreateDomains` (superseded by declared operations); the system pod's `transfer` and `transfer_object` functions (superseded by declared operations). `deleted_objects` stays, restricted to globally-executed transactions (section 3).
...
Mechanisms are proven by unit tests; behavior is proven end to end in the scenario corpus (`test/scenarios/`, per `test/TESTING.md`), which drives real node processes and reads structured events. Every new state mutation this design introduces (reparent, declared delete, domain register/renew/update/transfer/delete, the expiry sweep) therefore emits a typed `internal/events` constructor, so scenarios and production log consumers observe it; the domain event names are already reserved in the catalog.
...
- **Cascade and declared operations:** the permission walk resolves the controlling key through nested parents using tracker metadata only; subtree transfer changes only the root object; reparent rejects cycles, non-controlled targets, and version mismatches; deleting a parent with children is rejected; creation under a non-controlled object parent is rejected while creation under any KeyRoot succeeds (the gift); a pod output touching an existing parent is rejected; a pod output deleting a sharded object outside a globally-executed transaction is rejected while `merge` keeps working; the depth cap is enforced; ops-and-pod-call transactions are rejected.
