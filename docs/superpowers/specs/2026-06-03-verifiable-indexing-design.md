...
- The anchor is decided by evidence over one stake set, so the verdicts cannot fork: *support* is a round-N+1 vertex listing the anchor among its direct parents. The anchor is **certified** when supporters carry the stake quorum of round N+1, and **skipped** when non-supporters do; both quorums are over the same round's stake, so they intersect and are mutually exclusive. An anchor with split evidence is decided **indirectly** by the first later decided anchor: committed if it sits in that anchor's causal history, skipped otherwise, so an undecided round can never wedge the pipeline. During the relaxed bootstrap regime (before minimum validators, plus the transition window) the anchor is the round's lowest-hash vertex under the existing relaxed rule. Total latency stays in the N+2 envelope the current commit rule already has.
