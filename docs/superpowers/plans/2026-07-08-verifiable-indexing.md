...
- Pricing: `FeeParams` gains `DomainUpdateFee`/`DomainTransferFee`/`DomainDeleteFee` (defaults match `ReparentFee`/`DeleteFee`); `declaredOpFee` serves kinds 4-6. Completeness is guarded by a programmatic walk of the kind space (`TestEveryDeclaredOpIsPriced` enumerates up to `lastDeclaredOpKind`, a production constant in `domainops.go` that `stagedView.validate` also bounds — a future kind must bump it to apply at all, and bumping it unpriced reddens the walk); the four-site matrix checks site agreement only.
...
- As-built (7d10e39, re-review confirmed): `ValidateShape` carries three clauses (the empty-intent rule stays ingress-only — at commit it would newly reject transactions that pay and mutate nothing); the fail-safe is a `withheldStorage` parameter of `commitDeclaredOps` (keeps the unreachable branch drivable by tests); an ops+pod-call transaction, which previously committed as `declared_ops` after paying, is now `malformed_shape` and pays nothing — sound because the fee summary is producer/receiver agreement over headers while every accounting sink reads the real execution splits (re-derived exhaustively, mutation-verified); the gossip refusal emits `ingress.tx.rejected`.
...
**Waves B-D as-built (02bb806..ac3ab56, reviewed):** the epoch mirror publishes at four write sites (the genesis-regime refreeze was the fourth, found in implementation); `buildFeeSummary`/`validateFeeSummary` carry no explicit nil guard — enforcement is transitive through `calculateTxFeeSplit`, since a top-level panic would fire on every empty vertex a params-less test DAG produces; one-shot bpctl commands now load and persist the wallet file (the checkpoint lives there); `EnumerateSubtree` became a package function over `childrenSource` so the subtree verb reads proved under a checkpoint; the proved label reports only reads that actually verified, and a checkpoint-persist failure warns without discarding a verified answer. Deferred as design work, not cleanup: pod calls on nested objects (spec §3 gap, documented as-built), SDK `parent_kind`, podvm debug-wasm test lookup.
