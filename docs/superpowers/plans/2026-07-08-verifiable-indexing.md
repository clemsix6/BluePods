...
- [ ] **Run, expect PASS;** full package `go test ./internal/consensus/ -count=1 -timeout 300s` and fix tests that assumed arrival-order commits. Assert that after the first strict-regime direct certification, `fullQuorumAchieved` is set (production quorum and parent validation leave the relaxed posture).
