...
- **Known deferred items (explicitly out of scope, from the spec):** committed-tx-hash pruning, `SyncBufferSec` scaling rules, slashing consumption of fault evidence. The undecided-round fetch gap (I6) was closed during batch 0 (WAIT-stall frontier fetch). Remaining determinism follow-up (2026-07-14, batch-join debug): in the relaxed regime, `resolveIndirect` returns skip on a missing designated-producer vertex before consulting the certified anchor's causal completeness — a node with locally-incomplete history can skip where a fuller peer commits. Narrow window (relaxed only, further shrunk by the WAIT-fetch), not reproduced at HEAD; the candidate reorder (consult `causalBatch` first, WAIT on incomplete history) is a verdict-rule change requiring its own decision + TDD + divergence validation.
