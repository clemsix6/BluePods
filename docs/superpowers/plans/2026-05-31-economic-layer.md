...
- [ ] **Run** the system-pod build + `go test ./internal/consensus/ -run TestMint -count=1` (expect the mint path gone). Confirm the surviving balance-moving system functions conserve, and **fix the pre-existing `merge` inflation bug** surfaced here: `merge/execute.rs` sums input balances into the destination but never consumes the source coins (they keep their balances → duplication). `merge` must emit the source coins (1..N) in `deleted_objects` (or zero them) so total balance is conserved, and use a checked add to avoid `u64` wraparound. Add a `split`+`merge` conservation assertion (`sum(inputs) == sum(outputs)`); it must pass against the fixed `merge`.
...
**Files:** Modify `internal/consensus/fees.go` (split consumed vs storage); Modify `internal/consensus/commit.go` (`deductFees`, `calculateTxFee`); Modify `internal/state/state.go` (`computeStorageDeposit` reads a `validatorCount func() int`); Modify `internal/consensus/dag.go`/`cmd/node` wiring (set `state.validatorCount = dag.validators.Len`); Test `internal/consensus/fees_test.go`, `internal/consensus/commit_test.go`, `internal/state/state_test.go`.
...
- [ ] **Test:** for a create tx, `deductFees` debits `consumed+storage` from the coin but `epochFees` grows only by `consumed`; the created object's `fees` equals `storage`; after the commit, `sum(coins)+sum(object.fees)` is unchanged vs before (no supply created/destroyed at create). **Critical count-match cases (grow AND shrink):** register additional validators so `d.validators.Len()` exceeds the init-time count, create an object, assert `debited storage == stamped object.fees`; then remove a validator (deregistration applied) and create another object, asserting they STILL match (this is the case that fails with a stale/add-only counter). Also a producer→validator round-trip asserting `validateFeeSummary` accepts a vertex whose `buildFeeSummary` used consumed-only.
- [ ] **Run, expect FAIL.**
- [ ] **Implement:**
  1. **One validator-count source (not two synced counters).** Do NOT keep `state.totalValidators` as independent state updated by a callback — `ValidatorSet` has an `OnAdd` hook but NO removal hook (removals go through `applyPendingRemovals`→`Remove`, `epoch.go:138`), and `OnAdd` fires outside the commit lock, so any synced counter drifts after a deregistration. Instead give `state.State` a `validatorCount func() int` set once at wiring to `dag.validators.Len` (the SAME live set consensus reads), and have `computeStorageDeposit` call `s.validatorCount()`. Now `state` and consensus both read one source (`d.validators`), so they cannot diverge across adds OR removes, and it stays deterministic (every node applies the same committed validator-set changes in the same order). Within a single tx the count is stable for all current functions (a `register_validator` creates no objects, so no debit-vs-stamp skew); add a `// TODO` note if a future function both registers and creates.
...
**Files:** Modify `internal/sync/snapshot.go`; Modify `internal/consensus/dag.go` (`WithIssuanceRate` Option); Modify `cmd/node/sync.go` (carry the rate in `snapshotResult` and apply it at DAG construction); Test `internal/sync/snapshot_test.go`.

- [ ] **Test:** `issuance_rate_micro` round-trips, is checksum-covered (tamper test, like `total_supply`), and a DAG built from the synced result starts at the restored rate (not 0).
- [ ] **Run, expect FAIL.**
- [ ] **Implement:** thread `issuance_rate_micro` through `CreateSnapshot`/`computeChecksumWithInfo`/`verifyChecksum`. **Restore via a construction Option, not a setter:** for a syncing validator the DAG does NOT exist at `requestAndApplySnapshot` (it is created later in `initConsensusForValidator`, after `performSync`), so a `n.dag.SetIssuanceRate(...)` call there would nil-panic. Instead carry the restored rate in the `snapshotResult` struct and apply it as `consensus.WithIssuanceRate(rate)` where the other result-derived options are applied at DAG construction (both the validator and listener init paths). `WithIssuanceRate` seeds `d.issuanceRateMicro` (overriding the `genesisRateMicro` default). (`total_supply` (Task 2.6) is safe via the setter because `state` is DB-backed and reloaded by `state.New`, but the issuance rate lives only on the DAG, so it must ride a construction Option.)
- [ ] **Run** `go test ./internal/sync/ -count=1 -timeout 120s`.
...
**Review-fix coverage (iteration 3).** Storage count-sync via the add-only `OnAdd` callback (drifts after removals, fires off-lock) → 2.2 replaced with a single live source: `state.validatorCount func() int` bound to `dag.validators.Len`, so state and consensus read one set; test now covers grow AND shrink. Issuance-rate restore nil-panicked (DAG built after `requestAndApplySnapshot` on the sync path) → 6.5 restores via a `WithIssuanceRate` construction Option carried in `snapshotResult`, not a post-sync setter. Pre-existing `merge` inflation (sources never consumed) surfaced by 1.6's conservation assertion → 1.6 now fixes `merge` to delete its source coins + checked-add; 2.7 exercises split/merge.
...
**Residual risks.** (1) Repeated snapshot version bumps (6→7→8→9→10) — each must update encode+decode+checksum on BOTH the build and verify sides. (2) Rust system-pod rebuild required before Go-side bond/delegate handlers are exercised (Batches 3/4). (3) FlatBuffers regen (`bash types/generate.sh`) + rebuild before tests on Batches 2/6/8/9. (4) Batch 1.1 (commit-time authenticity) changes a hot path for every tx — verify sim throughput does not regress. (5) The storage-deposit count-sync (2.2) is determinism-critical: state reads the count from one live source (`dag.validators.Len`), not a synced counter — the count-match test must exercise BOTH a grown and a shrunk validator set. (6) Restore-on-sync of DAG-only state (issuance rate) must ride a construction Option, not a post-sync setter, because the DAG is built after the snapshot is applied on the validator/listener sync paths.
