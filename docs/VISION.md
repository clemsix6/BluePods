...
Versus Sui. The closest cousin: the object model, version-based conflict detection, and the DAG consensus lineage all come from this family. Sui keeps a consensus-less fast path for single-owner objects, which is faster on simple transfers but takes those transactions outside the global order, so the guarantee depends on which path a transaction took, and it puts large storage in a separate network (Walrus). BluePods folds storage and execution into one protocol and puts every transaction through the same order.

Versus Solana. Solana makes the same bandwidth bet, but it is leader-based and its roadmap moves toward isolating execution into domains that cannot touch each other's state, which is the opposite of letting one transaction reach any object in the system. BluePods is leaderless and keeps that reach global.
