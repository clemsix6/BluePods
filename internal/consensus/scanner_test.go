...
func TestScanDetectsNewHoldings(t *testing.T) {
	...
	obj1 := Hash{0x01}
	obj2 := Hash{0x02}
	dag.tracker.trackObject(obj1, 1, 3, 0, 0, Hash{})
	dag.tracker.trackObject(obj2, 1, 3, 0, 0, Hash{})
...
func TestScanMixedResults(t *testing.T) {
	...
	dag.tracker.trackObject(objKeep, 1, 3, 0, 0, Hash{})
	dag.tracker.trackObject(objFetch, 1, 3, 0, 0, Hash{})
	dag.tracker.trackObject(objDrop, 1, 3, 0, 0, Hash{})
	dag.tracker.trackObject(objIgnore, 1, 3, 0, 0, Hash{})
...
func TestScanBackground_DoesNotBlockCommit(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		id[0] = byte(i >> 8)
		id[1] = byte(i)
		dag.tracker.trackObject(id, 1, 3, 0, 0, Hash{})
	}
