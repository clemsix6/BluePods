...
// fallthrough for any node with no upstream — neither --bootstrap nor
// --bootstrap-addr — and no resumable local state, and marking on that
// branch too would launder a refused join's residue into adopted state; and
// in performSync, strictly after verifySyncedState returns nil, when a join
// completes verification (the snapshot has been attested by a stake quorum
// of the checkpointed validator set). A join the gate refuses never reaches
// it, which is what keeps the unverified snapshot such a join leaves on disk
// from being resumed as if it were the node's own.
